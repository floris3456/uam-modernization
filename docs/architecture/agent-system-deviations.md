...
| Promotion is a normal delegated task and assumes both pushes succeed | Promotion is a no-edit operation with exact pending-state evidence for resumable post-`main` synchronization | A normal task snapshot invalidates exact-SHA approval, a failed second push must not strand accepted `main`, and structural similarity alone cannot identify the exact initiated promotion. |
