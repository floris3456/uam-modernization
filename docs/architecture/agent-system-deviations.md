...
| Failed-push recovery handles only one-sided ancestry | Directed recovery preserves the marker's failed SHA, restores a discarded head only by safe fast-forward, and can create one exact-head, non-conflicting merge commit | The failure marker must block until synchronized history demonstrably includes the failed commit, while still permitting history-preserving recovery from real divergence. Conflicts and ambiguity fail closed. |
| Promotion is a normal delegated task and assumes both pushes succeed | Promotion is a no-edit operation with exact pending-state evidence retained across ambiguous push outcomes | A normal task snapshot invalidates exact-SHA approval, a failed second push must not strand accepted `main`, structural similarity alone cannot identify the exact initiated promotion, and a client-reported push failure does not prove the remote rejected the merge. |
