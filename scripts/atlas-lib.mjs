...
export async function validateModel(model, options = {}) {
  ...
  for (const gate of model.gates ?? []) {
    ...
    for (const field of ['name', 'diagramName', 'purpose', 'proofClaim', 'diagramSummary', 'whyNow', 'passCondition', 'stopCondition']) {
...
export function renderIndex(model) {
  ...
  const interactiveBody = body
    .replace(/<object class="diagram" id="gate-map-diagram"[\s\S]*?<\/object>/, `<div class="diagram" id="gate-map-diagram" aria-label="Dependency map of all 16 UAM proof gates">${renderSvgPreview(model).trim()}</div>`)
    .replace("mapDiagram?.addEventListener('load',()=>{try{mapDiagram.contentDocument?.addEventListener('wheel',onMapWheel,{passive:false})}catch{}});", '');
...
export function renderSvgPreview(model) {
  ...
  const nodes = model.gates.map(gate => {
    ...
    const titleLines = wrapSvgText(`${gate.id} · ${gate.diagramName}`, 25, 2);
    const claimLines = wrapSvgText(gate.diagramSummary, 36, 2);
