# Prompt 20 — Portal information architecture and administrative workflows

## Expert role

You are a enterprise product designer, accessibility specialist, and operations workflow architect. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `batches/05-portal-governance/20-portal-information-workflows/result-20-portal-information-workflows.md`.
...
This prompt belongs to **Portal authorization, workflows, and audit**. Run once control-plane concepts and data classifications are stable. Prompts 19–21 may run in parallel.
...
1. Design information architecture and task flows for fleet health, population/policy, application registry/rules/conflicts, task/release rollout, aggregates/detail, diagnostics/errors, audit, exports/deletion, integrations, and support.
2. Apply aggregate-first and least-detail defaults, impact preview, reason/approval, optimistic concurrency, draft/review/publish/rollback, staged rollout, kill switches, bulk-action safety, empty/error/stale/offline states, and safe recovery.
3. Define workflow ownership, status/state models, notification/escalation, accessibility to current WCAG/enterprise standards, localization/time zones, keyboard/screen-reader support, and performance budgets.
4. Separate browser UI from API authority; no generic direct table editor or hidden production actions.
5. Specify usability/accessibility tests with synthetic data and representative roles.
...
Actively search GitHub and official open-source sources for accessible admin portals, design systems, policy workflow UIs, fleet management interfaces. For every repository used, report:
...
- Screen-level data/action contracts
- Safety and recovery patterns
- Accessibility test matrix
- Phased portal backlog and acceptance criteria
...
- Terminology and accessibility policy
- Approval and notification responsibilities
- Whether any detail view is needed
...
- Run accessibility and authorization tests
- Measure task success and performance

Primary gate: **Normal administration is possible without direct DB access, generic row mutation, inaccessible controls, or unaudited high-risk action.**
