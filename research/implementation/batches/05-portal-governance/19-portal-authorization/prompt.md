...
You are a enterprise IAM authorization architect and privacy access-control engineer. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `batches/05-portal-governance/19-portal-authorization/result-19-portal-authorization.md`.
...
1. Define resources/actions/scopes/realms/purposes/conditions for fleet, policy, applications/rules, detail, aggregates, diagnostics, exports, tasks, releases, deletion, audit, and integrations.
2. Design capability RBAC plus limited ABAC, deny-by-default API enforcement, tenant isolation, separation of duties, JIT grants, approvals, purpose/ticket/target/time, break-glass, expiry/revocation, delegated administration, and service identities.
3. Define OIDC/BFF session/token/CSRF boundaries, authorization decision logging without sensitive payloads, caching invalidation, outage behavior, and emergency lockout recovery.
4. Specify positive/negative/property tests for every route/mutation and cross-realm confused-deputy cases.
5. Do not infer organization roles; provide a mapping workshop and safe provisional personas.
...
Actively search GitHub and official open-source sources for authorization policy engines, JIT access systems, OIDC BFF references, multi-tenant authorization test suites. For every repository used, report:
...
- Capability/resource/action schema
- Authorization policy model and decision flow
- Role/persona proposal clearly marked provisional
- JIT/approval/break-glass state machines
...
- Who receives each capability
- Approvers and separation of duties
- Break-glass authority and monitoring
- Permitted detail/export purposes
...
- Implement server-side negative tests before UI
- Generate synthetic realms/personas
- Test expiry/revocation/cache behavior

Primary gate: **Every API action is denied unless an explicit capability, realm, purpose, and condition permits it; cross-realm access tests are zero-tolerance.**
