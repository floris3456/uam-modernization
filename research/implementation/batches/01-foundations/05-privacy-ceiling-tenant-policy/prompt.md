# Prompt 05 — Product privacy ceiling and tenant-policy narrowing

## Expert role

You are a privacy-by-design architect and policy security engineer. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `batches/01-foundations/05-privacy-ceiling-tenant-policy/result-05-privacy-ceiling-tenant-policy.md`.
...
1. Define a release-authorized product privacy ceiling covering sources, fields, transformations, capabilities, destinations, diagnostic levels, and hard-deny classes.
2. Define typed signed tenant policy that may only narrow the ceiling, including scope, targeting, effective/expiry, revision, approval, rollback, offline cache, kill switches, invalid/unknown/expired behavior, and old-agent compatibility.
3. Specify where enforcement occurs in Task Host, User Host, Coordinator, server, portal, and release pipeline, and how no single compromised plane can broaden collection.
4. Define policy ownership, configuration provenance, feature flags, emergency disablement, policy simulation, impact preview, audit, and safe recovery.
5. Provide formalizable invariants and negative tests proving policy monotonicity.
...
Actively search GitHub and official open-source sources for policy engines, signed configuration systems, feature-flag kill-switch systems, policy-as-code test frameworks. For every repository used, report:
...
- Ceiling and tenant-policy schemas with fictional examples
- Authority/trust model and signing flow
- Policy evaluation and rollout state machines
- Monotonic-narrowing algorithm
- Failure/recovery matrix and canary tests
...
- Approved purposes, fields, sources, hard-deny categories, and identity/time precision
- Policy approvers and emergency authority
- Notice, consultation, and legal assessment
...
- Implement monotonicity/property tests
- Test expired/unknown/rollback policies
- Scan all sinks for forbidden canaries

Primary gate: **Invalid, expired, unsupported, or broader policy produces zero new collection and an audited, privacy-safe health state.**
