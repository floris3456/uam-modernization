# Prompt 08 — G4 URL canonicalization, application matching, and endpoint privacy transformation

## Expert role

You are a URL standards expert, application-security engineer, and privacy transformation architect. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `batches/02-endpoint-data/08-g4-url-privacy-transformation/result-08-g4-url-privacy-transformation.md`.
...
1. Define parsing/canonicalization order for schemes, hosts, IDNA/Unicode, case, trailing dots, ports, userinfo, percent encoding, paths, queries, fragments, control characters, invalid input, and public-suffix considerations.
2. Define endpoint allowlisting/site mapping and application-rule evaluation using accepted registry contracts, including ambiguity and versioned normalization.
3. Specify hard-deny detection, field minimization, time/identity reduction, rejected-value telemetry that never logs the value, and final privacy boundary placement.
4. Design a differential/property/fuzz test corpus covering encoding tricks, sensitive canaries, very large input, parser disagreement, and rule conflicts.
5. State when a custom implementation is too risky and which maintained libraries are suitable after license/security review.
...
Actively search GitHub and official open-source sources for URL parsers/canonicalizers, public suffix implementations, policy matchers, URL fuzz corpora. For every repository used, report:
...
- Normative canonicalization and transformation pipeline
- Input/output and rejection schemas
- Rule evaluation pseudocode/state flow
- Attack/failure matrix
- Million-case fuzz/canary plan
...
- Approved site/domain/path/time output
- Hard-deny categories
- Public-suffix and private-domain business semantics
...
- Implement golden vectors before production logic
- Differential-test candidate libraries
- Scan IPC/storage/log/network sinks for canaries

Primary gate: **No forbidden source value or reversible derivative crosses into Coordinator IPC, durable storage, diagnostics, or transport.**
