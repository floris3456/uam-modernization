# Prompt 10 — Model-based, property-based, fault-injection, and invariant testing

## Expert role

You are a principal verification engineer and reliability test architect. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `batches/03-durability-release-identity/10-invariant-fault-testing/result-10-invariant-fault-testing.md`.
...
1. Create a unified verification strategy for session isolation, privacy monotonicity, source cursor correctness, SQLite durability, idempotent receipts/materialization, update rollback, realm isolation, deletion/restore, and audit atomicity.
2. Choose model-based/property/fuzz/concurrency/fault tools for C#/.NET, Windows, SQLite, HTTP, and relational databases; define deterministic seeds, virtual clocks, shrink/replay, evidence, and CI versus lab lanes.
3. Specify injectable fault points at OS/process/file/transaction/network/worker/restore boundaries and avoid tests that only mock away the failure.
4. Define architecture fitness functions and release gates, including flaky-test policy and coverage limits.
5. Search open source for mature frameworks and real failure-injection examples, assessing maintenance and licenses.
...
Actively search GitHub and official open-source sources for FsCheck/Hedgehog-style property testing, state-machine model testing, fault-injection frameworks, Windows chaos tools, database Jepsen-style tests. For every repository used, report:
...
- Invariant catalogue with formal/plain definitions
- System models and command/event alphabets
- Fault matrix and hook placement
- Tool decision matrix
- CI/lab/pilot test pyramid
- Reproducible evidence format and ordered harness backlog
...
- Risk acceptance for untestable residuals
- Required evidence depth and release blocking policy
- Lab/physical hardware availability
...
- Implement models before components
- Run deterministic fault campaigns
- Store minimized failing seeds and evidence

Primary gate: **Every load-bearing invariant has an automated falsification path or an explicitly owned manual exercise.**
