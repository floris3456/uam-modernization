# Prompt 14 — Windows, browser, RDP/RDS, profile, ARM64, EDR, sleep, and compatibility policy

## Expert role

You are a Windows enterprise compatibility lead and browser platform engineer. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `batches/03-durability-release-identity/14-windows-compatibility-policy/result-14-windows-compatibility-policy.md`.
...
1. Define supported/conditional/test-only/unsupported matrices for Windows editions/releases/LTSC/Server, x64/ARM64, Edge/Chrome/Firefox channels, console/RDP/RDS/AVD, FSLogix/Citrix/roaming profiles, multi-session/same-SID, sleep/hibernate/Modern Standby, proxies/VPNs, Defender and third-party EDR.
2. Define qualification cadence, browser schema fixtures, OS servicing, runtime packaging, dependency native-architecture checks, EDR exclusions policy, support expiry, health reporting, and fail-closed behavior.
3. Separate VM-provable claims from physical hardware and enterprise-environment claims.
4. Specify compatibility test lanes, representative matrices, evidence, exception/waiver lifecycle, and customer readiness questionnaires.
5. Avoid claiming support from vendor documentation alone.
...
Actively search GitHub and official open-source sources for Windows compatibility test harnesses, browser matrix CI, FSLogix/RDS test examples, ARM64 packaging projects. For every repository used, report:
...
- Rolling compatibility policy and matrix
- Test-lane architecture
- Environment detection/health schema
- Qualification and exception lifecycle
- Failure/cleanup tests
- Minimum first-slice support claim
...
- Required customer/platform coverage
- Exception and deprecation policy
- Hardware/enterprise lab access
...
- Inventory lab harmless facts
- Run supported lanes
- Record unsupported states without silent success

Primary gate: **No platform is labeled supported until its security, acquisition, update, durability, resource, and cleanup gates pass.**
