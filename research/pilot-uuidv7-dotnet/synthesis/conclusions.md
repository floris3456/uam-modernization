...
| 2 | The built-in lacks the RFC 9562 §6.2 monotonic counter; same-millisecond UUIDs sort randomly | FACT (attributed) | Medium — fragmentation figures are third-party, not yet reproduced locally | [§Human summary](result-pilot-uuidv7-dotnet-synthesis.md#human-summary) | → ADR-FS-003 input | |
...
| 5 | The package format works end-to-end: card → study → web research → verbatim result → synthesis → conclusions → validation | FACT | High | [§Human summary](result-pilot-uuidv7-dotnet-synthesis.md#human-summary) | → disposition requested: pilot acceptance and workflow-contract proof are HUMAN decisions | |
