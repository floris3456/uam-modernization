# Research templates

- The [study template](study/) shows the shape for one bounded topic: `prompt.md`, `README.md`, and a `result-NN-<slug>.md` target. Copy it when scaffolding a new study.
- The [package-card template](package-card.md) shows the package card shape (status, type, output, ingredients).
- Use [the review checklist](review-checklist.md) before accepting a result.
- The prompt-authoring guide is the `prompt-authoring` skill (`.opencode/skills/research/prompt-authoring/SKILL.md`).
