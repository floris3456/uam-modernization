# Prompt 07 — G2/G3 Edge profile discovery, safe acquisition, source identity, and cursor correctness

## Expert role

You are a senior Windows browser-storage engineer and data-correctness specialist. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-02-endpoint-data/07-g2-g3-edge-acquisition-cursor-result.md`.
...
This prompt belongs to **Endpoint source and privacy transformation**. May run while G1 is implemented. Prompts 07–08 may run in parallel after the Batch 1 accepted baseline is available.
...
1. Specify supported Edge profile discovery, policy-defined roots, multi-profile handling, source IDs, source generations, same-SID concurrent sessions, profile removal/recreation, and unsupported/custom roots.
2. Compare short read-only transactions, SQLite Online Backup, and defer behavior under live writes and locks; define scratch ACLs, lifecycle, cancellation, cleanup, and proof of zero source writes.
3. Define schema capability detection and current/previous-version fixture strategy without treating browser internals as a stable public API.
4. Define native visit-ID/source-time cursor, bounded overlap, late synchronization, same timestamps, deletions, ID reuse/regression, DB replacement, clock/DST/sleep behavior, checkpoint atomicity, and deterministic dedupe input.
5. Specify the smallest G2/G3 VM prototypes and browser-impact measurements.
...
Actively search GitHub and official open-source sources for browser-history readers, Chromium history parsers, SQLite Online Backup examples, browser fixture generators. For every repository used, report:
...
- Profile/source identity schema
- Acquisition and cursor state machines
- SQLite query/backup safety rules
- Fixture/visit-generator design
- Failure and compatibility matrix
- Exact G2/G3 implementation backlog and pass/fail tests
...
- Supported Edge channels/versions and profile technologies
- First-run lookback and allowed time precision
...
- Create synthetic profiles and localhost visits
- Run lock/update/replacement/clock tests
- Use filesystem tracing to prove zero writes

Primary gate: **Failed/busy/unsupported/corrupt reads never advance; controlled fixture recall is complete; no cross-profile/session mix or source writes.**
