# Prompt 01 — G0 dummy-data and test-oracle architecture

## Expert role

You are a principal test-data architect, privacy engineer, and model-based testing lead. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-01-foundations/01-g0-dummy-data-test-oracle-result.md`.
...
1. Define three safe data tiers: fully fictional committed fixtures, sanitized organization-shaped internal data, and tightly controlled measured evidence.
2. Design canonical synthetic organization, role, persona, account, device, session, application, policy, event, batch, receipt, audit, deletion, migration, and expected-result models.
3. Define deterministic seeds, clocks, identifiers, distributions, scenario composition, data lineage, schema evolution, and a truth ledger that predicts accepted/rejected/quarantined outcomes and cursor progress.
4. Map datasets to G1–G12, including privacy canaries, cross-session attacks, browser locks, duplicates, outages, disk pressure, restore, and 6,000-device load.
5. Specify how the sanitized catalogue profile informs shape without copying internal values or inventing role mappings.
...
Actively search GitHub and official open-source sources for synthetic data generators, property-based test-data frameworks, model-based testing tools, privacy canary scanners. For every repository used, report:
...
- Logical and physical dummy-data schemas with example fictional records
- Deterministic generator and oracle algorithm
- Dataset catalogue by proof gate, size, classification, retention, and owner
- Canary/fuzz corpus and leak-scanning rules
- Data-quality validation and reproducibility tests
...
- Whether any real application names may be used internally
- Role/persona truth and access ownership
- Retention and controlled evidence policy
...
- Implement generator and schema after review
- Verify deterministic hashes
- Run canary scans and scenario reconciliation
