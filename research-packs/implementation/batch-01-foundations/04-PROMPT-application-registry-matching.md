# Prompt 04 — Application registry, aliases, ownership, and URL/process matching

## Expert role

You are a enterprise application-data architect and rule-engine designer. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-01-foundations/04-application-registry-matching-result.md`.
...
1. Design UAM-owned stable application identity, nullable/non-authoritative external keys, aliases, source provenance, lifecycle, ownership, sensitivity, categories, and CMDB links.
2. Design revisioned URL/domain and process/publisher/product/path rules with normalization versions, priority, specificity, effective dates, conflicts, shadowing, testing, approvals, rollback, and endpoint snapshot publication.
3. Define unknown/unmatched/ambiguous behavior and prohibit deriving real rules from names alone.
4. Define import staging and quality workflow for missing/duplicate external IDs and unsafe/truncated labels without exposing catalogue values.
5. Define rule simulation, impact preview, fixture generation, conflict detection, data lineage, and realm isolation.
...
Actively search GitHub and official open-source sources for policy/rule engines, URL and process matching systems, CMDB/application registries, rule-conflict analyzers. For every repository used, report:
...
- Registry and rule schemas
- Matching algorithm and deterministic precedence
- Rule-revision lifecycle/state machine
- Import/quality/conflict workflow
- Endpoint snapshot contract and size strategy
- Test matrix with fictional URL/process examples
...
- Application owners and source of truth
- Approved application categories and sensitivity
- Who may create, approve, publish, and retire rules
...
- Build sanitized import validator
- Generate fictional rules and conflicts
- Benchmark snapshot and matching performance

Primary gate: **No real rule is published without owner, purpose, revision, test examples, conflict result, approval, and rollback.**
