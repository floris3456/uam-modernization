# Prompt 03 — Contract, compatibility, and versioning strategy

## Expert role

You are a principal protocol architect and compatibility engineer. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-01-foundations/03-contract-versioning-result.md`.
...
- `04-data-and-schema-evidence-summary.md` — from the implementation package's `attachments/` directory
...
1. Define contract families for collector invocation/result, privacy transformation, IPC, policy/privacy ceiling, local events, batches, receipts, quarantine, health, audit, integration, and deletion tombstones.
2. Specify canonical serialization, schema identifiers, field presence/nullability, numeric/time/identifier rules, maximum sizes, unknown/duplicate field behavior, strictness, hashes/signatures, and test vectors.
3. Define backward/forward compatibility, supported-version windows, producer/consumer rollout order, N/N-1 or broader rules, deprecation, reservation, rollback, unsupported-schema behavior, and compatibility observability.
4. Prevent shared DTOs from coupling endpoint persistence, server storage, portal views, and external integrations.
5. Decide what needs a formal specification versus OpenAPI/JSON Schema/code generation, while leaving binary encoding measurement-gated.
...
Actively search GitHub and official open-source sources for schema registries, OpenAPI/JSON Schema compatibility tools, consumer-driven contract frameworks, canonical JSON implementations. For every repository used, report:
...
- Contract catalogue and ownership matrix
- Normative versioning rules
- Example schemas/messages and canonical test vectors
- Compatibility and rollout state machines
- Contract test strategy and architecture fitness functions
...
- Business semantic meaning of fields
- Compatibility duration and rollout policy
- External consumer commitments
...
- Implement schema linting and golden vectors
- Run old/new producer-consumer matrices
- Measure JSON/gzip before considering another encoding

Primary gate: **No component integration begins without a versioned contract, owner, size limits, compatibility rule, and rejection behavior.**
