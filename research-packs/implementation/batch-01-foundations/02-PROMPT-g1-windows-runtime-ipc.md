# Prompt 02 — G1 Windows Coordinator, User Host, Task Host, session identity, and IPC blueprint

## Expert role

You are a principal Windows security engineer and .NET endpoint architect. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-01-foundations/02-g1-windows-runtime-ipc-result.md`.
...
- `01-existing-system-evidence-summary.md` — from the implementation package's `attachments/` directory
...
This prompt belongs to **Immediate pre-implementation foundations**. Run first. Prompts 01–06 may run in parallel, then run the batch reviewer.
...
1. Specify the Coordinator Service, per-interactive-session User Host, and restricted Task Host processes down to executable ownership, lifecycle, startup, shutdown, recovery, and dependency rules.
2. Choose and justify service identity, service SID, privileges, filesystem/registry/certificate ACLs, Scheduled Task principal/logon/run-level/triggers, and behavior for console, lock, fast switch, RDP, disconnect, logoff, and same-SID sessions.
3. Define named-pipe or alternative IPC namespace, ACL construction, local-only enforcement, handshake, nonce, framing, version negotiation, caller token/SID/logon-SID/session/PID/creation-time validation, replay/PID-reuse defenses, cancellation, quotas, timeouts, malformed clients, and DoS containment.
4. Define Task Host token restriction, Job Object rules, inherited handles, network prohibition, filesystem capabilities, mitigations, kill-tree behavior, and residual limitations.
5. Produce a code-ready .NET solution blueprint and exact G1 hostile test plan without connecting to the lab.
...
Actively search GitHub and official open-source sources for Windows Service plus per-user agent implementations, secure named-pipe IPC libraries, Task Scheduler registration code, Windows Job Object and restricted-token sandboxes. For every repository used, report:
...
- Process and trust-boundary diagram
- Exact component/project responsibilities and allowed dependencies
- IPC wire protocol and state machine with example messages
- Windows object/ACL/privilege table
- Install/uninstall and failure-recovery sequence
- G1 test harness specification with commands, evidence, pass/fail, cleanup
...
- Supported Windows/session environments
- Enterprise Scheduled Task and service policy constraints
- Acceptable resource budget and support model
...
- Implement the smallest LocalService/User Host/hostile-client proof
- Use token/ACL/ProcMon evidence
- Run 10,000 cross-session and malformed-client attempts

Primary gate: **Zero cross-session accepted messages; no profile access by the service; no prohibited privileges; bounded hostile-client impact.**
