# Prompt 06 — Repository, solution, build, dependency, and CI architecture

## Expert role

You are a principal build/release engineer and secure software architect. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-01-foundations/06-repository-build-ci-result.md`.
...
1. Design the repository and .NET/frontend/tooling solution layout, project boundaries, dependency direction, shared-code limits, test layers, generated artifacts, configuration ownership, and architecture fitness functions.
2. Define reproducible builds, locked dependencies, analyzers, formatting, test isolation, secret scanning, license policy, SBOM, provenance, artifact naming, versioning, signing handoff, promotion, and branch protections.
3. Define Windows-specific CI needs, safe test certificates, emulator versus real-VM lanes, cache poisoning defenses, fork/PR trust, runner isolation, and least-privilege credentials.
4. Define secure coding/review standards, threat-model updates, dependency exception workflow, patch SLAs, and evidence retention.
5. Keep exact vendor/tool versions provisional and specify selection/update policies instead.
...
Actively search GitHub and official open-source sources for secure .NET monorepos, reproducible build systems, SBOM/provenance tooling, architecture test frameworks, hardened CI examples. For every repository used, report:
...
- Repository tree and project dependency diagram
- CI/CD lane design and trust boundaries
- Build/release artifact flow
- Quality gates and fitness functions
- Configuration/secrets matrix
- Ordered bootstrap backlog
...
- Hosting and CI platform policy
- Licensing/procurement constraints
- Branch/review/signing authority
- Staffing and supported toolchain
...
- Scaffold only accepted boundaries
- Add architecture dependency tests
- Prove clean reproducible build and SBOM generation

Primary gate: **No production artifact is promoted without reproducible inputs, review evidence, dependency/license checks, provenance, and separated signing authority.**
