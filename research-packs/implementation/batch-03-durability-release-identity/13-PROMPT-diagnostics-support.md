# Prompt 13 — Privacy-safe diagnostics, logging, support bundles, and temporary diagnostic access

## Expert role

You are a observability architect, privacy engineer, and enterprise support lead. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-03-durability-release-identity/13-diagnostics-support-result.md`.
...
1. Define endpoint/server/portal structured log and error taxonomies with bounded fields, correlation, fingerprints, severity, retryability, owner, user-safe messages, and no raw activity values.
2. Define OpenTelemetry metrics/traces and label-cardinality limits, sampling, propagation, storage, redaction, exception handling, crash behavior, and safe access logging.
3. Design support bundles using hashes/counts/status/configuration versions only, deterministic collection, size limits, consent/authorization, encryption, expiry, upload, access, deletion, and canary scanning.
4. Design temporary diagnostics policy with purpose/ticket/target/level/approval/expiry/kill switch and prevent it from exceeding the privacy ceiling.
5. Define incident response, runbooks, L1/L2/L3 ownership, supportability tests, and residual blind spots.
...
Actively search GitHub and official open-source sources for privacy-safe telemetry schemas, support bundle generators, OpenTelemetry collectors/processors, error fingerprint systems. For every repository used, report:
...
- Error taxonomy and structured event schemas
- Metric/trace catalogue with cardinality budget
- Support-bundle manifest
- Diagnostic-access state machine
- Redaction/canary tests
- Runbook and ownership catalogue
...
- Support roles and hours
- Diagnostic access authority
- Retention and approved support backend
...
- Seed canaries through all failure paths
- Measure cardinality and bundle size
- Test expiry and access revocation

Primary gate: **Support can diagnose defined failures without raw URLs, paths, identities, secrets, or unbounded telemetry.**
