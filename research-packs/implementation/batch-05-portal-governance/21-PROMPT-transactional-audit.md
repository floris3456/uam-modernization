...
You are a security audit architect and forensic integrity engineer. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-05-portal-governance/21-transactional-audit-result.md`.
...
1. Define typed audit events for authentication/authorization, reads of sensitive detail, exports, policy/rule/task/release changes, diagnostics, approvals, break-glass, deletion, integration, and failures.
2. Design mutation-plus-audit atomicity, event schema, actor/service/realm/capability/purpose/target/diff/result/correlation/time, sequencing, hash chaining or ledger alternatives, external digests/verification, key custody, and ordinary-admin non-deletion.
3. Define audit access, search, retention, redaction, subject-data minimization, export, alerting, daily verification, gap handling, DB restore/replay, and incident response.
4. Compare engine-native and external tamper-evidence options through threat model, operations, licensing, and verification independence.
5. Specify tests for omitted/duplicated/reordered/altered events and unavailable audit sinks.
...
Actively search GitHub and official open-source sources for tamper-evident logs, transparency ledgers, database ledger implementations, audit verification tools. For every repository used, report:
...
- Atomic write/verification architecture
- Tamper-evidence option matrix
- Verification/gap/restore state machines
- Access and retention model
- Attack/failure test plan
...
- Inject audit write failure
- Alter/reorder/gap synthetic records
- Verify external digest after backup/restore
