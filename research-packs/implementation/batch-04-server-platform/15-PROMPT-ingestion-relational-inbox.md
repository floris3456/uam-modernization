# Prompt 15 — Ingestion API, durable relational inbox, idempotency, leasing, quarantine, and materialization

## Expert role

You are a principal distributed-systems and relational ingestion architect. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-04-server-platform/15-ingestion-relational-inbox-result.md`.
...
1. Define gateway/API trust boundaries, mTLS-derived identity, header stripping, streaming compressed/decompressed limits, canonical batch hash, cheap synchronous validation, durable receipt semantics, duplicate same/different hash behavior, and receipt lookup.
2. Design engine-neutral relational inbox, immutable payload custody, worker leases/heartbeats, retries, poison/unsupported schema quarantine, event dedupe, materialization, aggregate projection, reconciliation, and terminal states.
3. Define transaction/failure boundaries for process/host/DB failover, lost responses, worker death after fact insert, replay after restore, backpressure, fairness, and realm isolation.
4. Specify schemas, indexes, state machines, observability/error taxonomy, operational tools, retention links, and integration outbox boundaries.
5. Evaluate external broker only against explicit measured break-even requirements.
...
Actively search GitHub and official open-source sources for transactional inbox/outbox implementations, idempotent ingestion APIs, relational job queues, worker lease libraries. For every repository used, report:
...
- HTTP/receipt contracts
- Inbox/dedupe/quarantine/fact schemas
- Worker lease and batch state machines
- Failure/recovery matrix
- Realm-isolation model
- G8 prototype and acceptance tests
...
- Durable failure domain and RPO/RTO
- Schema support horizon
...
- Implement contract emulator and poison corpus
- Kill API/worker/DB transitions
- Reconcile receipts to facts

Primary gate: **Every durable receipt remains recoverable and reaches one materialized effect or explicit terminal quarantine.**
