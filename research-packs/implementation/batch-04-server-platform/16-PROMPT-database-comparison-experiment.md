# Prompt 16 — PostgreSQL versus SQL Server experimental comparison

## Expert role

You are a independent database performance, operations, and cost evaluator. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-04-server-platform/16-database-comparison-experiment-result.md`.
...
1. Design a fair, engine-neutral experiment using identical semantic schemas, data, ingest/materialization logic, indexes, query corpus, retention, late arrivals, restore, failover, backup, observability, and hardware/service class.
2. Compare durable inbox leasing, bulk ingestion, dedupe/uniqueness with partitioning, WAL/log amplification, vacuum/index maintenance, partition operations, report coexistence, HA/DR, security/realm isolation, tooling, skills, licensing, managed/on-prem options, and on-call burden.
3. Define benchmark anti-patterns, warmup, repetitions, confidence intervals, reproducibility, cost normalization, and decision weighting/sensitivity.
4. Keep exact product versions current at execution and pin them in evidence rather than the architecture baseline.
5. Produce a decision gate, not a predetermined winner.
...
Actively search GitHub and official open-source sources for database benchmark harnesses, PostgreSQL/SQL Server bulk ingest examples, relational queue benchmarks, backup/restore test tooling. For every repository used, report:
...
- Comparable schema/workload specification
- Benchmark harness and runbook
- Metrics and statistical analysis plan
- Operations/skills/license/TCO matrix
- Failover/restore/retention tests
- ADR decision template with thresholds
...
- Strategic platform standards
- Budget/licensing assumptions
- Available on-call skills
- RPO/RTO and reporting priorities
...
- Run both candidates with identical generated data
- Publish raw reproducible measurements
- Choose only after gate

Primary gate: **Database ADR requires production-shaped results, restore/failover evidence, skills/TCO assessment, and owner approval.**
