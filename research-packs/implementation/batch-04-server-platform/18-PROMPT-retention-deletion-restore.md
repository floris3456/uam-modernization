# Prompt 18 — Retention, deletion, backup, restore, tombstones, exports, and integration deletion

## Expert role

You are a data lifecycle architect, privacy engineer, and disaster-recovery specialist. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-04-server-platform/18-retention-deletion-restore-result.md`.
...
1. Map lifecycle across endpoint outbox/grace, gateway/inbox/quarantine, facts/aggregates, caches, replicas, search, logs, audit, exports, integrations, backups, restored environments, and legal holds.
2. Design purpose/field-based retention policy representation, deletion requests, subject resolution, tombstone ledger, connector/export registry, retry/evidence, backup expiry, restore-time re-deletion, read-readiness gates, and acknowledged-event replay.
3. Reconcile immutable audit/evidence with erasure, minimize raw retention, and define cryptographic deletion/key-boundary limits.
4. Define failure/incident behavior for partial deletion, unavailable destination, restore before tombstones, stale caches, recipient copies, and accidental reingestion.
5. Separate technical mechanisms from human/legal retention decisions.
...
Actively search GitHub and official open-source sources for deletion/tombstone systems, backup restore validation, data lifecycle controllers, integration outbox deletion patterns. For every repository used, report:
...
- Store-by-store lifecycle matrix
- Retention/deletion/tombstone schemas
- Deletion and restore state machines
- Backup/integration/export contracts
- Failure and evidence matrix
- G11 drill and acceptance plan
...
- Purpose, legal basis, retention, legal hold, rights process
- RPO/RTO and backup policy
- Export/integration owner obligations
...
- Delete, restore older backup, replay tombstones and endpoint grace
- Verify no visibility before readiness
