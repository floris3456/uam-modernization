# Prompt 22 — Legacy consumer, report, script, schedule, credential, buffer, and integration discovery

## Expert role

You are a legacy modernization discovery lead and database dependency analyst. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-06-migration/22-legacy-discovery-result.md`.
...
1. Design a read-only discovery method for endpoint versions/settings/checkpoints/buffers, DB objects/logins/jobs/queries/reports, PSU routes/actions, scripts/schedules, HR/AD joins, exports, integrations, firewall paths, service accounts, manual workflows, and unreachable devices.
2. Define evidence/provenance, owner interviews, static and runtime observation, false-negative limits, dynamic SQL/ad-hoc consumer treatment, sensitivity-safe output, and how to prove absence over time.
3. Create a disposition model: preserve approved outcome, deliberately change, retire, investigate/quarantine; never execute deferred SQL or scripts during discovery.
4. Define credential/buffer classification and incident handling without copying secrets or raw activity.
5. Specify a traceability graph from legacy evidence to requirement, consumer, new component, test, migration, rollback, owner, and removal proof.
...
Actively search GitHub and official open-source sources for database dependency discovery, software composition/inventory tools, SQL workload analyzers, legacy migration traceability tools. For every repository used, report:
...
- Discovery inventory schema
- Read-only collection/query plan
- Consumer/credential/buffer traceability model
- Interview guides
- Coverage and confidence method
- Disposition backlog and stop conditions
...
- Named owners and approved purposes
- Authority to inspect systems
- Consumer criticality and retirement approval
- Handling of unreachable devices
...
- Run approved read-only inventory later
- Hash evidence and redact outputs
- Observe residual use over an agreed window

Primary gate: **No legacy behavior, data, credential, script, or consumer is migrated or removed without evidence, owner, disposition, validation, and rollback.**
