# Prompt 23 — Parallel-run comparison, reconciliation, compatibility projection, and migration validation

## Expert role

You are a data migration validation architect and statistical reconciliation lead. Produce a decision-ready, implementation-level research result for the next-generation UAM project. Use easy, precise language, but retain expert technical depth.

## Result target

Save the complete response as `results/batch-06-migration/23-parallel-run-reconciliation-result.md`.
...
1. Define one-authoritative-writer architecture, server-side compatibility projection, shadow data paths, and prohibition of endpoint dual-write.
2. Define exact invariants versus approved tolerances for counts, source ranges, keyed digests, timestamps/buckets, rule outcomes, checkpoints, aggregates, errors, and reports while minimizing sensitive detail duplication.
3. Design canonical comparison keys, known-defect register, mismatch taxonomy, sampling/break-glass, provenance, late data, retries, dedupe differences, timezone/identity differences, owner review, and automated pause/rollback thresholds.
4. Define configuration transformation validation and historical/read-only strategy.
5. Specify migration evidence and sign-off without treating legacy output as automatically correct.
...
Actively search GitHub and official open-source sources for data reconciliation frameworks, migration validation tools, CDC/compatibility projection references, privacy-preserving comparison techniques. For every repository used, report:
...
- Comparison contracts and digest schemas
- Invariant/tolerance and mismatch matrices
- Compatibility projector boundaries
- Reconciliation workflow/state machine
- Pilot gates and rollback triggers
...
- Known accepted legacy defects
- Tolerance owners
- Authority for sensitive break-glass comparison
...
- Use synthetic and approved aggregate fixtures
- Generate comparison digests
- Classify every material mismatch

Primary gate: **Every exact invariant passes; every tolerated mismatch has an owner-approved reason; endpoint dual-write remains impossible.**
